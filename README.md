...
## Generation, sessions, and realtime

See the [complete generation and sessions guide](docs/GENERATION.md) for the provider matrix, durable transports, ArkhamJS events, bearer sessions, and release policy.
...
`createVideoActions(flux).generateVideo({imageUrl, prompt, provider: 'higgsfield'})` requires an explicit provider and calls
...
Generation providers are explicit: generic content actions support `claude`, `gemini`,
and `openai`; image actions support `gemini`, `higgsfield`, and `openai`; video actions
support `gemini` (Veo) and `higgsfield`. No generation provider is inferred. The same
